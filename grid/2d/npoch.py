...
def aa():
    npoch = npp.empty(50000, dtype="int")
    sideface = npp.empty(3, dtype="int")

    si_file = open("sidegridt2.dat", "r")
    sicount = 0
    ...
    for i in range(nbp + 1, np):
        npoch[i] = 0

    sideface_file = open("sidefcae2.dat", "r")
    sidefacecount = 0
    for line in si_file.readlines():
        ...
        if sideface[2] != 1:
            npoch[sideface[0]] = sideface[2]
            npoch[sideface[1]] = sideface[2]
