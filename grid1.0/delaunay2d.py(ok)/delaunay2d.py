...
def main():
    ...
    global gx, gy, gcx, gcy, gcr, x, y, xce, yce  # gx,gy都是临时变量
    global ndel, edel, neibn
    ...
    # eop用来存放这个点的对应的单元的信息
    # bgeop这个点在背景网格的哪一个单元里面
    # pstep用来存放每一点的步长
    global bgnp, bgne, bgstep, bgfail, bgsearch, bgepoch
    global bgpx, bgpy, bgcx, bgcy, bgcr, bgnod, bgnoe
    # bgpx, bgpy,bgstep背景网格的每一个点的x坐标，y坐标以及步长
    # bgnod背景网格由哪三个点组成
    # bgnoe有哪三个相邻单元
    # bgcx, bgcy, bgcr背景网格对应的外接圆的圆心和半径
    np = 0
    i = 0
    j = 0
    k = 0
    ...
    print("boun_grid ok")
    for i in range(0, ne):
        ecrit[i] = 0   # 存放判断每一个单元是不是正常的数组，正常的就放一个正数，不正常的就放一个负数
        for j in range(0, 3):
            nei == noe[j, i]  # 每个单元三个相邻的单元是哪几个
            if nei == 0:
                ecrit[i] = 1
                break
    ...
    for i in range(0, np):
        xx = px[i]
        yy = py[i]
        ep, step = search_in_bg(xx, yy, 1, ep, step)  # 检查每一个边界点对应着背景网格的哪一个单元
        if ep == 0:
    ...
    while True:
        ...
        for i in range(0, ne):
            ...
            for j in range(0, 3):
                nei = noe[j, i]
                if nei == 0:  # 标记这个单元是不是正常，不正常就给它赋值为-1,noe是每个单元相邻三角形单元是哪几个
                    ecrit[i] = 1
                    break
                elif ecrit[nei - 1] == -1:  #？？？？？？？
                    ecrit[i] = 1
    ...
    gri = open("gridt.dat", "w")  # 把新生成的点的坐标写到这个文件中去
    for i in range(0, np):
...
def boun_grid():  # 把边界上的每一个点给引进来，作三角化，检查它的边界完整不完整，再把外部的四个点删除
    ...
    for line in sid.readlines():
        ...
        px[sidcount] = current_line[0]   # 读取边界点的X,y坐标
        ...
        if sidcount == 0:  # 找到边界点上x坐标与y坐标的最大值
            xmax = px[sidcount]
            xmin = px[sidcount]
            ymax = py[sidcount]
            ymin = py[sidcount]
    ...
    nps = np
    sid.close()
    print("read ok")  # 边界点读取完毕
    centx = (xmax + xmin) / 2  # 找到边界的中点
    centy = (ymax + ymin) / 2
    len_x = xmax - xmin  # 求边界的长度
    len_y = ymax - ymin
    length = max(len_x, len_y)
    length = 1.2 * length
    convex_hull()  # 先引入四个点把整个边界都给框起来，用凸多边形把边界包住
    ne = 2  # ne为当前单元数，四个点把边界框起来之后分成了两个三角形
    lastnp = -1  # 记录的上一个点
    nsearch = 0  # 记录找到的第n个边界点
...
def convex_hull():  # 先引入四个点把整个边界都给框起来，用凸多边形把边界包住
    ...
    i = 0
    j = 0
    ipx[0] = centx - 0.65 * length  # 记录外边框的那四个点的坐标
    ...
    for i in range(0, 2):
        for j in range(0, 3):
            gx[j] = ipx[-nod[j, i] - 1]  # 边框临时坐标，为了计算外接圆的圆心和半径的
            gy[j] = ipy[-nod[j, i] - 1]
...
def advance_front():  # 用前沿推进方法来生成节点
    ...
    f3 = 0.3
    nsearch = 0
    for i in range(0, ne):  # 给每个单元的属性赋初始值
        epoch[i] = 0
    bgsearch = 0
    for i in range(0, bgne):  # 给背景网格的每个单元辐初始值
        bgepoch[i] = 0
    newnp = np
    for i in range(0, ne):
        if ecrit[i] != 1:   # ecrit存放判断每一个单元是不是正常的数组，正常的就放一个正数，不正常的就放一个负数
            continue
        pp[0] = nod[0, i]
        pp[1] = nod[1, i]
        pp[2] = nod[2, i]
...
def background_grid():  # 读取背景网格的信息
    ...
    for line in bgg.readlines():
        ...
        bggcount += 1
    bgnp = bggcount  # 背景网格一共有多少个点，bgpx与bgpy是背景网格每个点的坐标
    bgscount = 0
    ...
    bgg.close()
    bgs.close()
    for i in range(0, bgne):  # 记录每个背景网格的单元中每个点的坐标
        for j in range(0, 3):
            gx[j] = bgpx[bgnod[j, i] - 1]
            gy[j] = bgpy[bgnod[j, i] - 1]
        circumcircle()  # 求背景网格中每个单元的外接圆的圆心及半径
        bgcx[i] = gcx
        bgcy[i] = gcy
        bgcr[i] = gcr
